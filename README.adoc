...
----
 ...
 ...
 Tests run: 3, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 2.128 s - in it.io.openliberty.guides.inventory.SystemResourceIT
...
Run the following command to start the PostgreSQL database, which runs the `postgres-sample` image in a Docker container, and map `5432` port from the container to your machine:
...
Begin by creating a RESTful client interface for the `inventory` microservice:
...
Next, create the `SystemData` data model, which represents each system in the `inventory` by holding the ID, hostname, operating system name, Java version, and heap size properties.
...
The [hotspot file=1]`SystemData` class encapsulates system properties and provides various [hotspot=getMethods file=1]get and [hotspot=setMethods file=1]set methods for accessing and modifying these properties.

=== Building test cases

Now you can start writing the test that uses the `SystemResourceClient` test REST client.
...
In this section, you will learn how to improve your integration tests by incorporating Testcontainers. This offers a more realistic, production-like environment for testing your application.

=== Building Testcontainer for Open Liberty

You'll start by defining a custom `LibertyContainer` class which provides a framework to start and access a containerized version of the Open Liberty application for testing.
...
The [hotspot=LibertyContainer file=0]`LibertyContainer` class extends the [hotspot=GenericContainer file=0]`GenericContainer` class from Testcontainers to create a custom container configuration specific to the Open Liberty application. The `GenericContainer` class provides a flexible API for configuring and launching arbitrary Docker containers, and your `LibertyContainer` subclass tailors this functionality to suit the specific needs of your application.
...
The `waitingFor()` method directs TestContainers to wait for a specific log message (`CWWKF0011I`) that indicates the Liberty server has started successfully.

Once the `LibertyContainer` is configured, you can now focus on setting up a logging system for your tests. Having reliable logs is essential for efficient debugging, as they provide detailed insights into the test execution flow, making it easier to identify and resolve issues during test failures. Testcontainers' built-in `Slf4jLogConsumer` enables integration of container output with the JUnit process, streamlining log analysis and simplifying test creation and debugging.
...
Define the `postgresContainer` test container to start up the PostgreSQL docker image, and define the `inventoryContainer` test container to start up the inventory docker image. 
...
The [hotspot=waitingFor file=2]`waitingFor()` method configures a custom readiness check for Testcontainers. It ensures that the application server is fully initialized and returns an HTTP 200 status code at the specified endpoint and port before allowing any tests to run. For different container readiness check customizations, refer to the https://www.testcontainers.org/features/startup_and_waits/[official Testcontainers documentation^] for full details.

Note that logger instances have already been created in both LibertyContainer.java and SystemResource.java files by using the LoggerFactory.getLogger() method. To integrate container logs with the logger instance, Testcontainers provides the withLogConsumer(new Slf4jLogConsumer(Logger)) method, which pipes the container output to the specified logger.

Update the `setup()` method to conditionally start the Testcontainers when running the tests and add the `tearDown()` method to stop the containers and close the network after running all tests.

=== Configuring Maven project

Now, configure your test build with Maven. 
...
Add the required `dependency` for Testcontainers and Log4J libraries with `test` scope. The [hotspot=testcontainers file=0]`testcontainers` dependency offers a general-purpose API for managing container-based test environments. The [hotspot=slf4j-log4j12 file=0]`slf4j-reload4j` dependency enables the Simple Logging Facade for Java (SLF4J) API for logging during test execution abd facilitates debugging and test performance tracking. 

Also, add the `maven-failsafe-plugin` plugin, so that the integration test can be run by the Maven `verify` goal.

Save the changes, and press the `enter/return` key in your console window to run the tests. You can see the tests still passed in dev mode or local runtime.
...
----
 ...
 Running it.io.openliberty.guides.inventory.SystemResourceIT
 ...
 Tests run: 3, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 30.425 s - in it.io.openliberty.guides.inventory.SystemResourceIT
...
Notice that the test initiates a new Docker container each for the PostgreSQL database and the application, resulting in a longer test runtime. Despite this, cold start testing benefits from a clean instance per run, ensuring consistent results. These tests also automatically hook into existing build pipelines that are set up to run the `integration-test` phase.
