...
The [hotspot=waitingFor file=2]`waitingFor(Wait.forHttp(String))` method configures a custom readiness check for Testcontainers. It ensures that the application server is fully initialized and returns an HTTP 200 status code at the specified endpoint before allowing any tests to run. For different container readiness check customizations, refer to the https://www.testcontainers.org/features/startup_and_waits/[official Testcontainers documentation^] for full details.
...
To use `Slf4jLogConsumer`, the necessary [hotspot=slf4j-log4j12 file=0]`slf4j-log4j12` dependency is already included in the Maven `pom.xml` file for you.
...
You just tested a reactive MicroProfile microservice using Testcontainers.
