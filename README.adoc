...
Because you started Open Liberty in dev mode, you can run the tests by pressing the `enter/return` key from the command-line session where you started dev mode. You see the tests passed.
...
Next, you will learn how to use Testcontainers to verify your microservice in the same Docker containers that you'll use in production.
...
The [hotspot=waitingFor file=0]`waitingFor()` method directs TestContainers to wait for the specific `CWWKF0011I` log message that indicates the Liberty server has started successfully.
...
The [hotspot=waitingFor file=0]`waitingFor()` method ensures the `inventoryContainer` is ready before tests run by checking the HTTP 200 status code at the specified `forHttp()` endpoint and `forPort()` port. For different container readiness check customizations, refer to the https://www.testcontainers.org/features/startup_and_waits/[official Testcontainers documentation^].
...
Save the changes, and press the `enter/return` key in your console window to run the tests. You still see the test passed.
...
----
 ...
 -------------------------------------------------------
 Running it.io.openliberty.guides.inventory.SystemResourceIT
 Testing by using Testcontainers...
 ...
...
Notice that the test initiates a new Docker container each for the PostgreSQL database and the application, resulting in a longer test runtime. Despite this, cold start testing benefits from a clean instance per run and ensures consistent results. These tests also automatically hook into existing build pipelines that are set up to run the `integration-test` phase.
