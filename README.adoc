...
Sometimes tests might pass in development and testing (dev/test) environments, but fail in production because the application runs differently in production than in testing (dev/test) environments. Fortunately, you can minimize these differences between dev/test and production by testing your application in the same Docker containers that you'll use in production.

=== What is Testcontainers?

Testcontainers is an open-source library that utilizes the docker-java library, which wraps the Docker in a Java API. Building on this, Testcontainers integrates into the lifecycle of test frameworks to provide containers as a resource at test time. This allows for consistent, portable testing environments especially when dealing with applications that have external resource dependencies such as databases, message queues, or web services. By encapsulating these dependencies in containers, Testcontainers ensures a uniform testing setup and simplifies the configuration process.
...
As you started Open Liberty in dev mode, all the changes were automatically picked up. You can run the tests by pressing the `enter/return` key from the command-line session where you started dev mode. You will see the following output:
