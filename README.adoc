...
Testcontainers is an open-source library that utilizes the docker-java library, which wraps Docker in a Java API. Building on this, Testcontainers integrates into the lifecycle of test frameworks to provide containers as a resource at test time. This allows for consistent, portable testing environments especially when dealing with applications that have external resource dependencies such as databases, message queues, or web services. By encapsulating dependencies in containers, Testcontainers ensures a uniform testing setup and simplifies the configuration process.
...
This guide uses Docker to run an instance of the PostgreSQL database for a fast installation and setup. A Dockerfile file is provided for you. Navigate to the `postgres` directory and run the following command to use the Dockerfile to build the image:
...
Next, you'll learn to write tests that use the `SystemResourceClient` REST client and the Testcontainers integration. You'll set up a multi-container test environment, determine the appropriate environment to launch Testcontainers, and get familiar with the workings of the test REST client.
...
Because containers are isolated by default, placing both the [hotspot=LibertyContainer file=0]`LibertyContainer` and the [hotspot=postgresContainer file=0]`postgresContainer` on the same [hotspot=network1 hotspot=network2 hotspot=network3 file=0]`network` allows them to communicate using the hostname `localhost` and the internal port `5432`, bypassing the need for an externally mapped port.

Note that the [hotspot=waitingFor file=0]`waitingFor()` method here overrides the [hotspot=waitingFor file=1]`waitingFor()` method from [hotspot file=1]`LibertyContainer`. Given that the `inventory` service depends on a database service, ensuring readiness involves more than just the microservice itself. To address this, the `inventoryContainer` readiness is determined by checking the [hotspot=waitingFor file=0]`/health/ready` health readiness check API, which reflects both the application and database service states. For different container readiness check customizations, refer to the https://www.testcontainers.org/features/startup_and_waits/[official Testcontainers documentation^].
